...
# ==============================================================================
# 🚢 CONFIGURAZIONE PLANCIA STREAMLIT (v51.0 - Edizione Estesa)
# ==============================================================================
st.set_page_config(
...
# 🔥 AUTO-RESET MEMORIA: Forza la transizione pulita dalla v50 se rileva vecchi log bloccati
if 'versione_corrente' not in st.session_state or st.session_state.versione_corrente != "51.0":
    ...
    st.session_state.log_sistema = ["Sistemi v51.0 riallineati. Scatola nera ripristinata e pulita."]
    ...
    for ticker in universo:
        try:
            if ticker in storico_universo.columns.levels[0]:
                ...
                # --- LOGICA 1: GESTIONE DEL TRAILING STOP ---
                ...
                # --- LOGICA 2: RILEVAMENTO NUOVI INGRESSI ---
                else:
                    if rsi_attuale <= soglia_rsi_fondo and var_percentuale < 0:
                        ...
                        if len(st.session_state.posizioni_attive) < max_posizioni:
                            ...
                            aggiungi_log(f"🚀 ORDINE ESEGUITO: Acquistato {ticker} a ${prezzo_attuale:.2f}")
        except:
            continue
...
with col_sx:
    ...
    if st.session_state.posizioni_attive:
        ...
        st.info("Nessun siluro in mare. Monitoraggio fondali attivo...")
